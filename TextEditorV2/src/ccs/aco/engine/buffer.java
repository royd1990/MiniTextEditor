...
public class buffer {
	 ...
	 * @param a This is a string type of parameter
	 * @param x	This is a integer type of parameter
	 */
	 ...
	 * of the buffer
	 * @return integer variable denoting position
	 */
	public int getCaret() {
		return caret;
	}
	 ...
	 * @param substring This is string type parameter.
	 * 
	 */
	public void bufferInsert(String substring){
		if(substring.length()!=0){
		//	System.out.println("Entered this section A "+substring.length());
			if(caret==0)
			{
				b = substring;
				caret = b.length()+1;
				...
				if(caret==b.length()+1){
					...
					if(selStop!=0){
						this.cutSpecial(); //Problem here: Write the full code here instead of calling a cut 
		  ...
		  // System.out.println("Entered this section B");
		   this.delete();
	   }
	   selStart = 0;
	   selStop = 0;
	...
	/**
	 * This is a method internally used by the buffer to handle the deletion operation
	 */
	private void delete(){
		if(selStop!=0){
			System.out.println("Entering subsection B.1.1");
			System.out.println("Caret "+caret);
			this.cut();
		}
		else{
			if(caret==(b.length()+1)){
				System.out.println("Entering subsection B.1.2");
				System.out.println("Caret "+caret);
	...
	/**
	 * This is a method used by internally the buffer
	 */
